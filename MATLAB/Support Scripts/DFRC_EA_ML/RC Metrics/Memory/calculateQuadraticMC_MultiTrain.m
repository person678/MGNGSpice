function [quadraticMCConfig] = calculateQuadraticMC_MultiTrain(gainInput, gainTransferFunction, SimulationConfig, MetricConfig)
...
for k = 1:quadraticMCConfig.memoryWidth
	...
	% Calculate weights
	outputWeights = trainFuncsMulti(trainStateMatrix,targetOutput);

	for n = 1:width(outputWeights')

		%% Apply weights 
		% Training set
		WeightedOutput_Training(:,n) = (outputWeights(n,:)*trainStateMatrix)';
		
		% Testing set
		WeightedOutput_Testing(:,n) = (outputWeights(n,:)*testStateMatrix)';
		
		%% Calculate Error
		Error_Training(n) = calculateError(WeightedOutput_Training(:,n),targetOutput, quadraticMCConfig);
		Error_Testing(n) = calculateError(WeightedOutput_Testing(:,n),outputVectorMemory_Testing(:,k), quadraticMCConfig);
				
	end

	%% Find best test result from results resting array
	[minimumValue, minimumIndex] = min(Error_Testing);

	% Generate the trained output
	trainedOutput(:,k) = WeightedOutput_Training(:,minimumIndex);
	testingOutput(:,k) = WeightedOutput_Testing(:,minimumIndex);

	% Calculate error
	quadraticMCConfig.trainingError(k) = Error_Training(minimumIndex);
	quadraticMCConfig.testingError(k) = Error_Testing(minimumIndex);
end
