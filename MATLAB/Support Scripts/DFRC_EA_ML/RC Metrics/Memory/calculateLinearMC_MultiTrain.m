function [linearMCConfig] = calculateLinearMC_MultiTrain(gainInput, gainTransferFunction, SimulationConfig, MetricConfig)
...
for k = 1:linearMCConfig.memoryWidth
	...
	for n = 1:width(outputWeights')
		...
		%% Calculate Error
		Error_Training(n) = calculateError(WeightedOutput_Training(:,n),targetOutput, linearMCConfig);
		Error_Testing(n) = calculateError(WeightedOutput_Testing(:,n),outputVectorMemory_Testing(:,k), linearMCConfig);
	...
	% Calculate error
	linearMCConfig.trainingError(k) = Error_Training(minimumIndex);
	linearMCConfig.testingError(k) = Error_Testing(minimumIndex);
end
