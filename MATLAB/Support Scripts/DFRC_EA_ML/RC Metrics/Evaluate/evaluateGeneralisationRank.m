function [] = evaluateGeneralisationRank(SimulationConfig)
...
if isfile([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/generalisationOutput_Vars.mat'])
   
    generalisationOutput_Vars = importdata([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/generalisationOutput_Vars.mat']);
    generalisationOutput = importdata([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/generalisationOutput.mat']);
    SimulationStep = length(generalisationOutput_Vars) + 1;
else
    SimulationStep = 1;
end
...
for k = SimulationStep:TotalSimulations
	...
	calcOutput = calculateRanks(variables(k).InputScalling,variables(k).TransferFunctionGain,SimulationConfig,MetricConfig,'generalisationRank');

	generalisationOutput_Vars(k) = variables(k);
    generalisationOutput(k) = calcOutput;
	
	save([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/generalisationOutput_Vars.mat'],'generalisationOutput_Vars','-v7.3');
...
save([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/MetricConfig_GR.mat'],'MetricConfig','-v7.3');
