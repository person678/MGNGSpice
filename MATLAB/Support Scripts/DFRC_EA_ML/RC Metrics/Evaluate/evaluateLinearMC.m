function [] = evaluateLinearMC(SimulationConfig)
...
if isfile([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/LinearMCOutput_Vars.mat'])
   
    LinearMCOutput_Vars = importdata([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/LinearMCOutput_Vars.mat']);
    LinearMCOutput = importdata([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/LinearMCOutput.mat']);
	SimulationStep = length(LinearMCOutput_Vars) + 1;
else
    SimulationStep = 1;
end
...
for k = SimulationStep:TotalSimulations
	...
	calcOutput = calculateLinearMC(variables(k).InputScalling, variables(k).TransferFunctionGain, SimulationConfig, MetricConfig);
	
	LinearMCOutput_Vars(k) = variables(k);
	LinearMCOutput(k) = calcOutput;
	
	save([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/LinearMCOutput_Vars.mat'],'LinearMCOutput_Vars','-v7.3');
...
save([fullfile('./ParamaterSweeps',SimulationConfig.dataset,SimulationConfig.timeStamp), '/MetricConfig_LMC.mat'],'MetricConfig','-v7.3');
